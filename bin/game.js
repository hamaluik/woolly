...
var mammoth_Input = function() {
	this.mouseDown = false;
	this.mouseY = 0;
	this.mouseX = 0;
};
mammoth_Input.__name__ = ["mammoth","Input"];
mammoth_Input.prototype = {
	init: function() {
...
mammoth_gl_js_Graphics.prototype = {
	...
	,checkWindowSize: function() {
		...
		if(this.context.canvas.width != displayWidth || this.context.canvas.height != displayHeight) {
			...
			mammoth_Log.log("Resized canvas to " + displayWidth + "x" + displayHeight,mammoth_LogFunctions.Debug,{ fileName : "Graphics.hx", lineNumber : 82, className : "mammoth.gl.js.Graphics", methodName : "checkWindowSize"});
			mammoth_Log.log("Mammoth size: " + this.context.drawingBufferWidth + "x" + this.context.drawingBufferHeight,mammoth_LogFunctions.Debug,{ fileName : "Graphics.hx", lineNumber : 83, className : "mammoth.gl.js.Graphics", methodName : "checkWindowSize"});
		}
	}
...
mammoth_loader_Loader.loadMesh = function(me) {
	var mesh = new mammoth_render_Mesh(me.name,mammoth_Mammoth.gl,me.vlayout);
...
mammoth_loader_Loader.loadObject = function(parentTransform,object) {
	...
	if(object.render != null && object.render.shader != null) {
		...
		var material = new mammoth_render_Material(object.render.mesh + "->" + object.render.shader,mammoth_Mammoth.gl);
		...
		while(_g2 < _g11.length) {
			...
			case "uv":
				...
				break;
			default:
				throw new js__$Boot_HaxeError(new mammoth_debug_Exception("Unknown vertex attribute '" + attribute + "'!",false,"UnknownAttribute",null,{ fileName : "Loader.hx", lineNumber : 154, className : "mammoth.loader.Loader", methodName : "loadObject"}));
			}
		}
		material.compile();
		var offset = 0;
...
mammoth_loader_Loader.load = function(file) {
	mammoth_Log.log("Loading data from " + file.meta.file + "..",mammoth_LogFunctions.Info,{ fileName : "Loader.hx", lineNumber : 225, className : "mammoth.loader.Loader", methodName : "load"});
...
var mammoth_render_Material = function(name,graphics) {
...
mammoth_render_Material.prototype = {
	compileShader: function(type,source) {
		...
		if(!this.context.getShaderParameter(shader,35713)) {
			var info = this.context.getShaderInfoLog(shader);
			var typeStr = type == 35633 ? "Vertex" : "Fragment";
			throw new js__$Boot_HaxeError(new mammoth_debug_Exception(info,true,"Compile" + typeStr + "Shader",null,{ fileName : "Material.hx", lineNumber : 61, className : "mammoth.render.Material", methodName : "compileShader"}));
		}
		return shader;
	}
	...
	,compile: function() {
		...
		if(!this.context.getProgramParameter(this.program,35714)) {
			...
			throw new js__$Boot_HaxeError(new mammoth_debug_Exception(info,true,"LinkProgram",null,{ fileName : "Material.hx", lineNumber : 94, className : "mammoth.render.Material", methodName : "compile"}));
		}
		return this;
	}
	...
	,bindAttributes: function() {
		...
		while(name.hasNext()) {
			...
			attribute.location = this.context.getAttribLocation(this.program,name1);
	...
	,bindUniforms: function() {
		...
		while(name.hasNext()) {
			...
			uniform.location = mammoth_render_TLocation.Uniform(this.context.getUniformLocation(this.program,name1));
			uniform.bound = true;
		}
	}
	,applyCullMode: function() {
		...
		case 0:
			...
			break;
		case 1:
			this.context.enable(2884);
			this.context.cullFace(1028);
			break;
		case 2:
			this.context.enable(2884);
			this.context.cullFace(1029);
			break;
		case 3:
			this.context.enable(2884);
			this.context.cullFace(1032);
			break;
		}
	}
	,applyDepthMode: function() {
		...
		this.context.depthFunc(tmp);
		this.context.depthMask(this.depthWrite);
	}
	...
	,apply: function() {
		...
		while(uniform.hasNext()) {
			...
			case 0:
				var b = _g1[2];
				this.context.uniform1i(location,b ? 1 : 0);
				break;
			case 1:
				var i = _g1[2];
			...
			case 3:
				var y = _g1[3];
				var x1 = _g1[2];
				this.context.uniform2f(location,x1,y);
				break;
			case 4:
				var z = _g1[4];
				var y1 = _g1[3];
				var x2 = _g1[2];
				this.context.uniform3f(location,x2,y1,z);
				break;
			case 5:
				var w = _g1[5];
				var z1 = _g1[4];
				var y2 = _g1[3];
				var x3 = _g1[2];
				this.context.uniform4f(location,x3,y2,z1,w);
				break;
			case 6:
				var x4 = _g1[2];
				this.context.uniform1fv(location,x4);
				break;
			case 7:
				var v = _g1[2];
				this.context.uniform2f(location,v[0],v[1]);
				break;
			case 8:
				var v1 = _g1[2];
				this.context.uniform3f(location,v1[0],v1[1],v1[2]);
				break;
			case 9:
				var v2 = _g1[2];
				this.context.uniform4f(location,v2[0],v2[1],v2[2],v2[3]);
				break;
			case 10:
				var m = _g1[2];
				this.context.uniformMatrix4fv(location,false,m);
				break;
			case 11:
				var c = _g1[2];
				this.context.uniform3f(location,c[0],c[1],c[2]);
				break;
			case 12:
				var c1 = _g1[2];
				this.context.uniform4f(location,c1[0],c1[1],c1[2],c1[3]);
				break;
			}
		}
		this.bindAttributes();
...
var mammoth_render_Mesh = function(name,graphics,attributeNames) {
	this.name = name;
	this.context = graphics.context;
...
mammoth_render_Mesh.prototype = {
	setVertexData: function(data) {
		this.context.bindBuffer(34962,this.vertexBuffer);
		this.context.bufferData(34962,new Float32Array(data),35044);
		this.context.bindBuffer(34962,null);
	}
	,setIndexData: function(data) {
		this.context.bindBuffer(34963,this.indexBuffer);
		this.context.bufferData(34963,new Int16Array(data),35044);
		this.context.bindBuffer(34963,null);
...
mammoth_systems_RenderSystem.prototype = {
	update: function(camera) {
		...
		var g = mammoth_Mammoth.gl;
		g.context.viewport(vpX,vpY,vpW,vpH);
		g.context.scissor(vpX,vpY,vpW,vpH);
		var colour = camera.clearColour;
		g.context.clearColor(colour[0],colour[1],colour[2],colour[3]);
		g.context.clear(16640);
		...
		while(o.hasNext()) {
			...
			material.apply();
			g.context.bindBuffer(34962,mesh.vertexBuffer);
			var _g = 0;
			...
			while(_g < _g1.length) {
				...
				g.context.vertexAttribPointer(attribute.location,tmp,5126,false,attribute.stride,attribute.offset);
			}
			g.context.bindBuffer(34963,mesh.indexBuffer);
			g.context.drawElements(4,mesh.vertexCount,5123,0);
...
mammoth_Mammoth.input = new mammoth_Input();
